...
We present a novel approach based on the well-known Hit-and-Run algorithm (used in geometric computation) to extract the structure of the feasible activations that produce various finger tip forces into the distal direction.
...
We use a realistic model of a static human index finger with 7 muscles, 4DOF, and 4 output dimensions.
For a progression of increasing distal force (from 10\% to 100\% of maximal), we examine the distribution across each muscle's activation, as the feasible activation space shrinks to the unique maximal solution.
...
Finally, we apply six non weighted and weighted static force cost functions and compare their effect upon the shape of the activation space.
