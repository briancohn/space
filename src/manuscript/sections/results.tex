...
The density integrals perpendicular to each muscle are unimodal due to the convexity\cite{ball1997elementary}, and therefore it would be inadvisable to fit a normal distribution as the probability density function.
...
We also placed a vertical grey line for the scaled unique solution at maximal force, denoted $\textbf{a}^*$, (e.g. LUM converges to an activation of 1 at maximal palmar force, so we put a grey line at 0.8 for $\alpha=0.8$ of maximal palmar force).
...
Unlike the solution projections, which only show unidimensional information about a set of sampled solutions, we use parallel coordinate visualization to view how different parts of one muscle's distribution interact with another's.
