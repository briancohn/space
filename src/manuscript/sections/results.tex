...
Next, we see that if one muscle had to be fixed throughout the entire force progression, di and pi would fail; their bounding boxes at a 10\% task do not include the unique solution at 100\%. We also placed a vertical grey line for the scaled unique solution at maximal, linearly interpolated for the given alpha level (e.g. lum converges to an activation of 1 at maximal palmar force, so we put a grey line at 0.8 for 80\% of maximal palmar force.)
...
On the histograms you can predict how many solutions would be lost if you put one constraint on one muscle. Parallel coordinates allow us to visualize not only the amount of solutions lost, but the location of the remaining solutions across the other muscle dimensions.
...
When ei is constrained below 60\% activation, we see that the bounding box of pi is significantly restricted; however, the same constraint has no effect on the bounding box of eip.
